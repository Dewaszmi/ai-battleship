...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.actor = nn.Sequential(
            nn.Linear(conv_out_size, 64), nn.Tanh(), nn.Linear(64, n_actions)
        )

        self.critic = nn.Sequential(
            nn.Linear(conv_out_size, 64), nn.Tanh(), nn.Linear(64, 1)
        )
    ...
    def get_action_and_value(
        self, x, action=None, mask=None, deterministic: bool = False
    ):
        x = x.unsqueeze(0) if x.dim() == 3 else x
...
def train_loop(config):
    ...
    envs = gym.vector.SyncVectorEnv(
        [
            make_env(args.env_id, i, args.capture_video, run_name, config=config)
    ...
    next_obs, infos = envs.reset(
        seed=args.seed
    )  # retrieve infos at the beginning for the masking
                ...
                if isinstance(infos, dict) and "action_mask" in infos:
                    ...
                    for info in infos_iter:
                        if info is not None and "action_mask" in info:
                            mask_list.append(
                                np.array(info["action_mask"], dtype=np.int8)
                            )
                        else:
                            mask_list.append(
                                np.ones(envs.single_action_space.n, dtype=np.int8)
                            )
